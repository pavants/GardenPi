...
class ZoneThread( threading.Thread ):
  ...
  print GPIOPin
  #logFilePath = "/data/gardenpi/irrigationLog.txt"
  #logFileHTML = "/var/www/raspberry/irrigationLog.txt"
  ...
  def run(self):
    ...
    if True:
      ...
      mx = ["start",str(self.zoneID), str(boardPin), time.strftime("%H:%M:%S %d-%m-%Y")]
      print mx[1]
      self.writeLog(mx)
      ...
      mx = ["end",str(self.zoneID), str(boardPin), time.strftime("%H:%M:%S %d-%m-%Y")]
      #Write event in Logger
      self.writeLog(mx)
  ...
  def writeLog(self,msg):
    ...
    logFile.write("<TR><TD CLASS='"+msg[0]+"'>"+msg[0]+"</TD><TD>"+msg[2]+"</TD><TD>"+msg[3]+"</TD></TR>\n")
    logFile.close()
